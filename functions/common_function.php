...
function get_unique_brands() {
    ...
    if(isset($_GET['brand'])) {
        $brand_id = $_GET['brand'];
            ...
            $num_of_rows = mysqli_num_rows($result_query);
            if($num_of_rows == 0) {
                    echo "<h3 class='text-center text-danger'>'This brand is not available for service.'</h3>"; 
            }
            // $row = mysqli_fetch_assoc($result_query);
            // echo $row['product_title'];
            while($row = mysqli_fetch_assoc($result_query)) {
...
function search_product() {
    ...
    $num_of_rows = mysqli_num_rows($result_query);
    if($num_of_rows == 0) {
            echo "<h3 class='text-center text-danger'>'No result match. No product found in this category.'</h3>"; 
    }
    while($row = mysqli_fetch_assoc($result_query)) {
    $product_id = $row['product_id'];
    $product_title = $row['product_title'];
...
function cart() {
    ...
    if($num_of_rows > 0) {
        ...
        echo "<script>alert('An item is added to cart.')</script>";
        echo "<script>window.open('index.php', '_self')</script>"; 
            ...
            if(!isset($_GET['my_order'])) {
                if(!isset($_GET['delete_account'])) {
                    ...
                    if($row_count > 0) {
                        ...
                        echo "<h3 class='text-center text-success mt-5 mb-2'>You have 0 pending order</h3>
